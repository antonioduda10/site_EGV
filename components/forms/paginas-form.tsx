...
export function PaginasForm() {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setStatus(response.ok ? "Página criada." : data.error ?? "Erro ao salvar página.");
      ...
      setStatus("Erro de conexão ao salvar página.");
      setStatusType("error");
    } finally {
      setIsSaving(false);
    }
  ...
  return (
    <form onSubmit={handleSubmit} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
      <div className="flex flex-col gap-1 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <h2 className="text-lg font-semibold text-slate-900 dark:text-slate-100">Nova página institucional</h2>
          <p className="mt-1 text-sm text-slate-600 dark:text-slate-300">
            Cadastre uma página com endereço público e conteúdo completo.
          </p>
        </div>
      ...
      <div className="mt-5 grid gap-3 md:grid-cols-2">
        <div className="space-y-1.5">
          ...
          <input name="titulo" placeholder="Ex: Nossa escola" required className={fieldClass} disabled={isSaving} />
        </div>
        <div className="space-y-1.5">
          <label className="text-sm font-medium text-slate-700 dark:text-slate-200">Endereço (slug)</label>
          <input name="slug" placeholder="Ex: escola" required className={fieldClass} disabled={isSaving} />
          <p className="text-xs text-slate-500 dark:text-slate-400">Use letras minúsculas e hífen. Ex: escola, equipe-diretiva.</p>
        </div>
      </div>

      <div className="mt-4 space-y-2">
        <label className="text-sm font-medium text-slate-700 dark:text-slate-200">Conteúdo</label>
        <PaginaEditor value={conteudo} onChange={setConteudo} />
      </div>
      <button
