...
export const metadata = createPublicMetadata({
  ...
  description: "Informações sobre o uso de cookies no Portal EGV.",
  path: "/cookies"
});

export default async function CookiesPage() {
  // Busca o texto da politica de cookies configurado no painel.
  const config = await db.configuracaoSite.findFirst({
    select: { cookies: true }
  }).catch(() => null);
  const conteudo = rewriteUploadUrlsInHtml(config?.cookies ?? "<p>Informações sobre o uso de cookies no portal.</p>");
  return (
